      ...
      <div className="w-full max-w-4xl p-6">
        ...
        {result && (
          <div className="space-y-8">
            ...
            {/* Upload-only response (deployed backend may just return upload info) */}
            {result.upload && (
              <div className="bg-white p-6 rounded-2xl shadow-lg text-center">
                <h2 className="text-xl font-semibold">Upload Successful</h2>
                <p className="mt-2">{result.upload.message}</p>
                {result.upload.url && (
                  <p className="mt-2 text-blue-600 break-all">URL: {result.upload.url}</p>
                )}
                <div className="mt-4">
                  <button
                    onClick={() => navigate("/")}
