...
TESTCASE_DIR = PROJECT_BACKEND_ROOT / "route_optimizer" / "testcases"
...
def upload_excel(request):
    ...
    if testcase_name:
        ...
        if not testcase_path.exists():
            return JsonResponse(
                {"error": f"Testcase {testcase_name} not found"},
                status=400,
            )
        ...
        return JsonResponse(
            {"error": "No file uploaded or testcase selected"},
            status=400,
        )
    ...
    try:
        ...
        return JsonResponse(
            {"error": "Optimization failed", "message": str(exc)},
            status=500,
        )
    finally:
...
def add_entity(request):
    ...
    try:
        ...
        return JsonResponse(
            json.loads(json.dumps({"status": "success", "data": result}, default=str))
        )
